...
const MessageHeader = ({ matchInfo, onBackClick, actions }: MessageHeaderProps) => {
  // If matchInfo is null, show a placeholder
  const profileName = matchInfo?.profile?.name || 'Loading...';
  const profileImage = matchInfo?.profile?.images?.[0] || '/placeholder.svg';
  const isVerified = matchInfo?.profile?.verified || false;
  ...
  return (
    <div className="border-b p-3 flex items-center justify-between bg-background/80 backdrop-blur-sm sticky top-0 z-10">
      ...
      <div className="flex items-center space-x-1">
        {actions}
        <Button variant="ghost" size="icon" className="text-love" disabled={!matchInfo}>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" disabled={!matchInfo}>
